...
<section class="w-full h-full relative">
	...
	>
		...
		<g class="animate-move-to-top">
			<g
				stroke-linecap="round"
				fill="none"
				stroke-miterlimit="10"
				class="animate-write-fill stroke-base-content"
				stroke-width="2"
			>
				<path
			...
			<text
				font-family="Praise"
				font-size="100"
				class="fill-base-content animate-fade-in opacity-0 -translate-x-6"
				y="62"
			>
				atthew
			</text>
	...
	<div class="w-full flex place-items-center justify-center">
		{#if ready}
			<div class="flex flex-row flex-wrap gap-4">
				<a
					class="btn btn-md md:btn-lg btn-circle btn-github animate-scale"
				...
				>
					<svg viewBox="0 0 16 16" class="fill-current w-6 h-6 md:w-8 md:h-8">
				...
				<a
					class="btn btn-md md:btn-lg btn-circle btn-linkedin animate-scale"
				...
				>
					<svg
						...
						class="fill-current w-6 h-6 md:w-8 md:h-8"
					>
	...
	<div class="absolute bottom-8 left-0 right-0 grid place-items-center">
		<a href="#about" class="hover:translate-y-1 transition-all duration-300">
...
<section
	class="bg-base-200 grid grid-cols-2 justify-center place-items-center gap-24"
	id="about"
>
	<div class="grid w-full justify-right text-center">
		<div class="text-5xl">
			<span use:reveal={animate(300)} class="font-bold block">
				Some of my work
			</span>
			<span use:reveal={animate(500)}>(keep scrolling)</span>
		</div>
	</div>
</section>
...
{#each projects as project, i}
	...
	>
		{#if i % 2}
			<div
				class="flex flex-col place-items-end text-right gap-4 h-full lg:py-16"
			>
				<span
					use:reveal={animate(300)}
					class="text-5xl max-w-xl mt-auto font-bold block"
				>
					{project.title}
				</span>
				<div class="flex flex-wrap max-w-md gap-2 flex-row-reverse">
				...
				<span
					class="text-lg max-w-xl lg:my-auto line-clamp-5"
		...
		{:else}
			...
			>
				<span
					use:reveal={animate(500)}
					class="text-5xl max-w-xl font-bold block lg:mt-auto"
				>
					{project.title}
				</span>
...
<style>
	...
	.animate-write-fill {
		animation: write-fill 1.2s linear forwards,
			shift-left 1s cubic-bezier(0.4, 0, 0.2, 1) forwards;
	...
	.animate-scale {
		transform: scale(0.2);
		animation: scale 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
	}
	...
	.animate-move-to-top {
		animation: move-to-top 0.8s cubic-bezier(0.4, 0, 0.2, 1) forwards;
	...
	section {
		@apply h-screen snap-start snap-always flex;
	}
